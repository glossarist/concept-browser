...
async function processDataset(dir, register, opts) {
  ...
  // Aggregate bulk exports offered on the dataset page: single-file
  // distributions generated by this build plus any files shipped in the
  // dataset's compiled/ directory. site-config `bulk_exports` restricts
  // which formats are offered; absent/empty config offers all available.
  const BULK_FORMAT_BY_EXT = {
    '.ttl': 'turtle',
    '.jsonld': 'jsonld',
    '.xml': 'tbx',
    '.jsonl': 'jsonl',
  ...
  const bulkFormats = [];
  const addBulkFormat = file => {
    const dest = path.join(DATA, register, file);
    if (!fs.existsSync(dest) || bulkFormats.some(f => f.file === file)) return;
    const formatName = BULK_FORMAT_BY_EXT[path.extname(file)] || path.extname(file).slice(1);
    bulkFormats.push({ file, format: formatName, size: fs.statSync(dest).size });
  };
  for (const file of [`${register}.csv`, `${register}.jsonld`]) addBulkFormat(file);
  ...
  if (fs.existsSync(compiledDir)) {
    ...
    for (const file of fs.readdirSync(compiledDir)) {
      fs.copyFileSync(path.join(compiledDir, file), path.join(DATA, register, file));
      addBulkFormat(file);
      copied += 1;
    }
    if (copied) console.log(`  Copied ${copied} bulk format files from compiled/`);
  }
  ...
  // Runs before the manifest write so the ttl distribution is on disk when
  // bulkFormats is assembled.
  await writeDatasetRdf(register, manifest, concepts, refMaps, opts);
  addBulkFormat(`${register}.ttl`);
  manifest.bulkFormats = opts.bulkExports?.length
    ? bulkFormats.filter(f => opts.bulkExports.includes(f.format))
    : bulkFormats;
...
// site-config `bulk_exports`: which aggregate formats the dataset pages
// offer for download. 'ttl' is accepted as an alias of the canonical
// format id 'turtle'. Absent/empty → offer all available formats.
const bulkExports = (config.bulkExports || []).map(f => (f === 'ttl' ? 'turtle' : f));
